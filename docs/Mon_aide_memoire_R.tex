...
\StringTok{      }\KeywordTok{rowwise}\NormalTok{() }\OperatorTok{\%\textgreater{}\%}\StringTok{ }\CommentTok{\# Groupement par ligne}
\StringTok{      }\CommentTok{\# Pour chaque colonne ayant les puces, on remplace la puce}
\StringTok{      }\KeywordTok{mutate}\NormalTok{(}\KeywordTok{across}\NormalTok{(}\KeywordTok{where}\NormalTok{(}\OperatorTok{\textasciitilde{}}\StringTok{ }\KeywordTok{any}\NormalTok{(}\KeywordTok{grepl}\NormalTok{(}\StringTok{"•"}\NormalTok{, .))), replace\_bullet)) }\OperatorTok{\%\textgreater{}\%}
...
\begin{center}\includegraphics[width=0.6\linewidth]{Mon_aide_memoire_R_files/figure-latex/unnamed-chunk-11-1} \end{center}

C'est beaucoup mieux. Néanmoins il reste une colonne en trop dans le tableau
\texttt{jura\_data{[}{[}3{]}{]}}. S'il n'y avait eu que des cases vides, la colonne aurait été supprimée. On peut donc regarder les lignes où il y a des erreurs :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{line\_id \textless{}{-}}\StringTok{ }\KeywordTok{which}\NormalTok{(}\OperatorTok{!}\KeywordTok{is.na}\NormalTok{(jura\_data\_cleaned[[}\DecValTok{3}\NormalTok{]]}\OperatorTok{$}\NormalTok{x))}

\KeywordTok{my\_reactable}\NormalTok{(}
\NormalTok{  jura\_data\_cleaned[[}\DecValTok{3}\NormalTok{]][line\_id,],}
  \DataTypeTok{searchable =} \OtherTok{FALSE}\NormalTok{,}
  \DataTypeTok{defaultPageSize =} \KeywordTok{length}\NormalTok{(line\_id), }
  \DataTypeTok{minRows =} \KeywordTok{length}\NormalTok{(line\_id)}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=0.6\linewidth]{Mon_aide_memoire_R_files/figure-latex/unnamed-chunk-12-1} \end{center}

Une seule ligne en ressort : celle pour le Grand Tétras. En comparant avec la ligne du tableau sur le PDF, on peut se rendre compte qu'il y a un décalage à partir de la colonne \texttt{directive\_oiseaux\_annexe\_1}. Cela peut être dû à la mauvaise visualisation de la puce (bien présente) à la colonne \texttt{protection\_nationale}. Cette puce ne semble pas visible, mais en surlignant avec la souris le texte de cette case, on peut voir apparaître la puce ``•''. On peut donc réécrire correctement cette ligne d'après le PDF :

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Suppression de la colonne en trop.}
\NormalTok{jura\_data\_cleaned[[}\DecValTok{3}\NormalTok{]] \textless{}{-}}\StringTok{ }\KeywordTok{select}\NormalTok{(jura\_data\_cleaned[[}\DecValTok{3}\NormalTok{]], }\OperatorTok{{-}}\NormalTok{x)}

\CommentTok{\# Modification de la ligne posant problème.}
\NormalTok{jura\_data\_cleaned[[}\DecValTok{3}\NormalTok{]][line\_id,] \textless{}{-}}\StringTok{ }\KeywordTok{list}\NormalTok{(}
...
Ensuite, on peut fusionner les deux sous-parties du tableau 3 (tableaux
...
\begin{center}\includegraphics[width=0.6\linewidth]{Mon_aide_memoire_R_files/figure-latex/unnamed-chunk-15-1} \end{center}
