...
\begin{center}\includegraphics[width=0.6\linewidth]{Mon_aide_memoire_R_files/figure-latex/unnamed-chunk-8-1} \end{center}
...
\begin{center}\includegraphics[width=0.6\linewidth]{Mon_aide_memoire_R_files/figure-latex/unnamed-chunk-10-1} \end{center}
...
\begin{center}\includegraphics[width=0.6\linewidth]{Mon_aide_memoire_R_files/figure-latex/unnamed-chunk-14-1} \end{center}
...
\begin{center}\includegraphics[width=0.6\linewidth]{Mon_aide_memoire_R_files/figure-latex/unnamed-chunk-16-1} \end{center}

\label{tab:mammifere-jura}Données présentant les mammifères de la Réserve Naturelle Nationale de la Haute Chaîne du Jura.

\begin{center}\includegraphics[width=0.6\linewidth]{Mon_aide_memoire_R_files/figure-latex/unnamed-chunk-17-1} \end{center}

\label{tab:avifaune-jura}Données présentant l'avifaune de la Réserve Naturelle Nationale de la Haute Chaîne du Jura.

\begin{center}\includegraphics[width=0.6\linewidth]{Mon_aide_memoire_R_files/figure-latex/unnamed-chunk-18-1} \end{center}
...
Le \emph{web scraping} est une technique permettant d'extraire, le plus souvent de
façon automatique, le contenu d'une page web. Cela peut être très utile pour
récupérer des données publiques nécessaires à nos analyses.

Sous R, le package permettant de parcourir et de récupérer facilement le contenu
d'une page web s'appelle \href{https://github.com/tidyverse/rvest}{\texttt{rvest}}.
...
\ref{tab:avifaune-jura}). Cette classification peut être facilement récupéré
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{get\_bird\_classification \textless{}{-}}\StringTok{ }\ControlFlowTok{function}\NormalTok{(bird, }\DataTypeTok{taxon =} \KeywordTok{c}\NormalTok{(}\StringTok{"Ordre"}\NormalTok{, }\StringTok{"Famille"}\NormalTok{)) \{}
\NormalTok{  wiki\_link \textless{}{-}}\StringTok{ "https://fr.wikipedia.org/wiki/Wikip\%C3\%A9dia:Accueil\_principal"}
\NormalTok{  session \textless{}{-}}\StringTok{ }\KeywordTok{html\_session}\NormalTok{(wiki\_link)}
  
\NormalTok{  form \textless{}{-}}\StringTok{ }\KeywordTok{html\_form}\NormalTok{(session)}
\NormalTok{  form\_modified \textless{}{-}}\StringTok{ }\KeywordTok{set\_values}\NormalTok{(form[[}\DecValTok{1}\NormalTok{]], }\DataTypeTok{search =}\NormalTok{ bird)}
  
  \CommentTok{\# On entre sur la page wikipédia de l\textquotesingle{}oiseau recherché.}
\NormalTok{  wiki\_bird \textless{}{-}}\StringTok{ }\KeywordTok{submit\_form}\NormalTok{(session, form\_modified, }\DataTypeTok{submit =} \StringTok{"go"}\NormalTok{)}
  
\NormalTok{  tbl\_classification \textless{}{-}}\StringTok{ }\NormalTok{wiki\_bird }\OperatorTok{\%\textgreater{}\%}
\StringTok{    }\KeywordTok{html\_node}\NormalTok{(}\DataTypeTok{xpath =} \StringTok{"//table[@class=\textquotesingle{}taxobox\_classification\textquotesingle{}]"}\NormalTok{) }\OperatorTok{\%\textgreater{}\%}
\StringTok{    }\KeywordTok{html\_table}\NormalTok{() }\OperatorTok{\%\textgreater{}\%}
\StringTok{    }\NormalTok{magrittr}\OperatorTok{::}\KeywordTok{set\_colnames}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\StringTok{"niveau\_classification"}\NormalTok{, }\StringTok{"nom\_classification"}\NormalTok{)) }\OperatorTok{\%\textgreater{}\%}
\StringTok{    }\KeywordTok{filter}\NormalTok{(niveau\_classification }\OperatorTok{\%in\%}\StringTok{ }\NormalTok{taxon) }\OperatorTok{\%\textgreater{}\%}
\StringTok{    }\NormalTok{tidyr}\OperatorTok{::}\KeywordTok{pivot\_wider}\NormalTok{(}
      \DataTypeTok{names\_from =}\NormalTok{ niveau\_classification,}
      \DataTypeTok{values\_from =}\NormalTok{ nom\_classification}
\NormalTok{    )}
  
\NormalTok{  tbl\_classification}\OperatorTok{$}\NormalTok{nom \textless{}{-}}\StringTok{ }\NormalTok{bird}
  
  \KeywordTok{return}\NormalTok{(tbl\_classification)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{noms\_latin \textless{}{-}}\StringTok{ }\NormalTok{stringr}\OperatorTok{::}\KeywordTok{str\_extract\_all}\NormalTok{(avifaune\_jura}\OperatorTok{$}\NormalTok{nom\_latin, r}\StringTok{"(\^{}\textbackslash{}w+\textbackslash{}s+\textbackslash{}w+)"}\NormalTok{)}

\NormalTok{classification\_oiseaux \textless{}{-}}\StringTok{ }\NormalTok{purrr}\OperatorTok{::}\KeywordTok{map}\NormalTok{(noms\_latin, get\_bird\_classification) }\OperatorTok{\%\textgreater{}\%}
\StringTok{  }\KeywordTok{bind\_rows}\NormalTok{() }\CommentTok{\# Fusion des tableaux}
\end{Highlighting}
\end{Shaded}
...
\begin{center}\includegraphics[width=0.6\linewidth]{Mon_aide_memoire_R_files/figure-latex/unnamed-chunk-23-1} \end{center}
...
{\centering \includegraphics[width=1\linewidth]{Mon_aide_memoire_R_files/figure-latex/plot-rnn-1} 
