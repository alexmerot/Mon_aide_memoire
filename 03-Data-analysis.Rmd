...
clean_extracted_data <- function(
  ...
  if (recode.bullet) {
    ...
    cleaned_df <- cleaned_df %>%
      ...
      # Pour chaque colonne ayant les puces, on remplace la puce
...
`jura_data[[3]]`. S'il n'y avait eu que des cases vides, la colonne aurait été supprimée. On peut donc regarder les lignes où il y a des erreurs :

```{r}
line_id <- which(!is.na(jura_data_cleaned[[3]]$x))

my_reactable(
  jura_data_cleaned[[3]][line_id,],
  searchable = FALSE,
  defaultPageSize = length(line_id), 
  minRows = length(line_id)
)
```

Une seule ligne en ressort : celle pour le Grand Tétras. En comparant avec la ligne du tableau sur le PDF, on peut se rendre compte qu'il y a un décalage à partir de la colonne `directive_oiseaux_annexe_1`. Cela peut être dû à la mauvaise visualisation de la puce (bien présente) à la colonne `protection_nationale`. Cette puce ne semble pas visible, mais en surlignant avec la souris le texte de cette case, on peut voir apparaître la puce "•". On peut donc réécrire correctement cette ligne d'après le PDF :

```{r}
# Suppression de la colonne en trop.
jura_data_cleaned[[3]] <- select(jura_data_cleaned[[3]], -x)

# Modification de la ligne posant problème.
jura_data_cleaned[[3]][line_id,] <- list(
...
On commence par ouvrir la session de la page web de l'accueil de Wikipedia.
Ensuite, on récupère

```{r}
get_bird_classification <- function(bird) {
  session <- html_session("https://fr.wikipedia.org/wiki/Wikip%C3%A9dia:Accueil_principal")
  
  form <- html_form(session)
  form_modified <- set_values(form[[1]], search = bird)
  
  # On entre potentiellement directement sur la page wikipédia de l'oiseau recherché.
  wiki_bird <- submit_form(session, form_modified, submit = "go")
  
  # On récupère le titre de 1er niveau de la page.
  title <- wiki_bird %>%
    html_node(xpath = "//div[@class='entete']//div") %>%
    html_text()

  # Si la recherche n'a pas trouvé directement la page de l'oiseau en question,
  # on tombe sur la page "Résultats de la recherche". On choisit donc de suivre
  # le 1er lien présenté.
  if (title == "Résultats de la recherche") {
    wiki_bird <- wiki_bird %>%
      follow_link(xpath = "//ul[@class='mw-search-results']//li[1]//div[1]//a[1]")
    
    # On récupère le bon titre (nom de l'oiseau) :
    title <- wiki_bird %>%
  ...
  if (is.na(tbl_classification)) {
    print("PAS DE TABLE DE CLASSIFICATION")
  } else {
    tbl_classification <- tbl_classification %>%
      html_table() %>%
  ...
  tbl_classification$nom_wiki <- title
