...
```{r fig-rnn, fig.cap='(ref:plot-rnn)', message=FALSE, warning=FALSE, out.width="100%", cache = TRUE}
layout_design <- c(
...
```{r plot-rnn-jura-interactive, cache=TRUE, message=FALSE, warning=FALSE}
library("ggiraph")

rnn_france <- ggplot(regions) +
  annotation_map_tile(zoom = 5, zoomin = 0, type = "stamenwatercolor") +
  geom_sf_interactive(
    aes(tooltip = nom, fill = nom, data_id = code_insee),
    colour = "black",
    alpha = 0.6,
    show.legend = FALSE
  ) +
  ...
  geom_point_interactive(
    data = tibble::rowid_to_column(data_rnn),
    aes(x = X, y = Y, tooltip = NOM_SITE, data_id = glue("point_{rowid}")),
    colour = "darkred",
    size = 2
  ) +
  annotation_north_arrow(
...
interactive_plot <- rnn_france + rnn_jura + 
  plot_layout(design = layout_design) +
  plot_annotation(caption = "\u00a9 OpenStreetMap contributors")

girafe(
  code = print(interactive_plot),
  options = list(
