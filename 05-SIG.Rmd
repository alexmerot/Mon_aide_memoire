...
Pour commencer, nous allons prendre pour exemple le jeu de données `world` du
package `sf`. Ce jeu de données provenant de [Natural
Earth](https://www.naturalearthdata.com/) est un objet `sf` contenant la carte
du monde ainsi que quelques données de banques mondiales (tableau
\@ref(tab:dataworld)).

(ref:dataworld) Aperçu du jeu de données `world` inclus dans le package `spData`. 

```{r dataworld}
knitr::kable(head(world), caption = "(ref:dataworld)") %>%
  kableExtra::kable_styling(
    latex_options = c("striped", "hovered", "condensed", "responsive")
  ) %>%
