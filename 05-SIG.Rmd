...
(ref:plot-rnn) (A) Carte des Réserves Naturelles de la France métropolitaine. (B) Carte montrant la délimitation (en rouge) de la Réserve Naturelle Nationale de la Haute Chaîne du Jura.

```{r plot-rnn, fig.cap='(ref:plot-rnn)', fig.show = "hold", out.width="100%", message=FALSE, warning=FALSE}
library("ggplot2")
...
# On zoome sur la carte des régions en augmentant les limites précédentes par 10 km.
crop_regions <- st_crop(regions, crop_limits + c(-0.1, -0.1, 0.15, 0.1))

rnn_jura <- ggplot() +
  annotation_spatial(crop_regions) +
